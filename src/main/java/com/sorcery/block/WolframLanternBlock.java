...
import com.sorcery.item.CrystalItem;
...
public class WolframLanternBlock extends Block
{
    ...
    public WolframLanternBlock()
    {
        super(Properties.create(Material.ROCK).hardnessAndResistance(hardness, resistance).sound(SoundType.STONE).setLightLevel((x) -> 15));
        this.setDefaultState(this.stateContainer.getBaseState().with(HANGING, false).with(COLOR, CrystalColor.NONE));
    }
